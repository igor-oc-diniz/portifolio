...
export const commands: Command[] = [
  // ── help ──────────────────────────────────────────────────────────────────
  {
    ...
    execute: () => [
      ...
      { type: 'accent',    content: `  ${pad('help', 14)}${pad('?', 12)}List all available commands` },
      { type: 'accent',    content: `  ${pad('whoami', 14)}${pad('about', 12)}About me — name, role and status` },
      { type: 'accent',    content: `  ${pad('skills', 14)}${pad('ls', 12)}Tech stack by category` },
      { type: 'accent',    content: `  ${pad('experience', 14)}${pad('exp', 12)}Career history with key metrics` },
      { type: 'accent',    content: `  ${pad('contact', 14)}${pad('', 12)}LinkedIn, GitHub and email` },
      { type: 'accent',    content: `  ${pad('open [target]', 14)}${pad('', 12)}Open linkedin | github in browser` },
      { type: 'accent',    content: `  ${pad('banner', 14)}${pad('', 12)}Display the welcome banner` },
      { type: 'accent',    content: `  ${pad('clear', 14)}${pad('cls', 12)}Clear terminal output` },
      { type: 'separator', content: SEP },
      { type: 'text',      content: '  💡 Try: whoami · skills · experience · contact' },
    ],
  },
  ...
  // ── whoami ─────────────────────────────────────────────────────────────────
  {
    ...
    execute: () => [
      ...
      { type: 'highlight', content: '🏢  Current role:  Senior Frontend Developer @ CI&T' },
      { type: 'highlight', content: '🚀  Venture:       Co-Founder & Mobile Engineer @ TopCaixa' },
      { type: 'highlight', content: '✅  Status:        Open to work' },
      { type: 'highlight', content: '🌐  Languages:     PT-BR · EN' },
      { type: 'highlight', content: '🎓  Education:     Computer Science — UFLA (2014–2022)' },
    ],
  },
  ...
  // ── skills ─────────────────────────────────────────────────────────────────
  {
    ...
    execute: () => {
      ...
      skillCategories.forEach((cat) => {
        lines.push({ type: 'highlight', content: `\n  ${categoryIcon(cat.label)}  ${cat.label}` })
  ...
  // ── experience ─────────────────────────────────────────────────────────────
  {
    ...
    execute: () => {
      const lines: OutputLine[] = [
        { type: 'accent',    content: `📋  Professional Experience  (${experiences.length} entries)` },
        { type: 'separator', content: SEP },
      ]
      experiences.forEach((exp, i) => {
        ...
        lines.push({ type: 'accent',    content: `  ${exp.period} · ${exp.sector}` })
        if (topMetric) {
          lines.push({ type: 'accent',  content: `  → ${topMetric.value} ${topMetric.label}` })
        }
    ...
    execute: (args?: string[]) => {
      ...
      if (target === 'linkedin') {
        if (typeof window !== 'undefined') {
          window.open('https://www.linkedin.com/in/igorotavio96/', '_blank', 'noopener,noreferrer')
        }
        return [{ type: 'success', content: '🚀  Opening LinkedIn profile...' }]
      }
      if (target === 'github') {
        if (typeof window !== 'undefined') {
          window.open('https://github.com/igor-oc-diniz', '_blank', 'noopener,noreferrer')
        }
...
export function getEasterEggOutput(): OutputLine[] {
  return [
    ...
    { type: 'accent',    content: '  ╚═╝  ╚═╝╚═╝╚═╝  ╚═╝╚══════╝   ╚═╝     ╚═╝╚══════╝ ╚═╝', delay: 30 },
    { type: 'text',      content: '' },
    { type: 'text',      content: "  sudo: permission denied — but let's talk anyway." },
    { type: 'separator', content: SEP },
    { type: 'text',      content: '  I\'m actively looking for my next challenge as' },
    ...
    { type: 'separator', content: SEP },
    { type: 'success',   content: '  Great engineers get hired. Let\'s make it happen. 🚀' },
  ]
}
